...
layui.define(['laytpl', 'laypage', 'layer', 'form'], function(exports) {
	...
	var $ = layui.$
		...
		, table = {
			...
			// 远程数据请求 ADD BY ZHANGCC
			, getRemoteData: function(func) {
				this.config.remoteFunc = func;
			}
		}
		...
		, ELEM_VIEW = 'layui-table-view', ELEM_HEADER = '.layui-table-header', ELEM_BODY = '.layui-table-header',
		ELEM_MAIN = '.layui-table-header',
		ELEM_FIXED = '.layui-table-fixed', ELEM_FIXL = '.layui-table-fixed-l', ELEM_FIXR = '.layui-table-fixed-r',
		ELEM_TOOL = '.layui-table-tool', ELEM_SORT = '.layui-table-sort', ELEM_EDIT = 'layui-table-edit', ELEM_HOVER = 'layui-table-hover'

		//thead区域模板
		, TPL_HEADER = function(options) {
			options = options || {};
			return ['<table cellspacing="0" cellpadding="0" border="0" class="layui-table" width="{{d.data.width||\"100%\"}}" '
				...
				, '{{# if(item2.hidden==="true") {return true;}  }}'
				, '{{# if(item2.checkbox){ }}'
				, '<th data-field="{{ item2.field||i2 }}" data-type="checkbox" unresize="true" width="{{item2.width}}"><div class="layui-table-cell laytable-cell-checkbox"><input type="checkbox" name="layTableCheckbox" lay-skin="primary" lay-filter="layTableAllChoose" {{# if(item2[d.data.checkName]){ }}checked{{# }; }}></div></th>'
				, '{{# } else if(item2.space){ }}'
				, '<th data-field="{{ item2.field||i2 }}" unresize="true"><div class="layui-table-cell laytable-cell-space" width="{{item2.width}}"></div></th>'
				, '{{# } else { }}'
				, '<th data-field="{{ item2.field||i2 }}" {{#if(item2.colspan){}} colspan="{{item2.colspan}}"{{#} if(item2.rowspan){}} rowspan="{{item2.rowspan}}"{{#}}} {{# if(item2.unresize){ }}unresize="true"{{# } }}  width="{{item2.width}}">'
				, '{{# if(item2.colspan > 1){ }}'
		...
		//tbody区域模板
		//, TPL_BODY = ['<tbody></tbody>'].join('')

		//主模板
		,
		TPL_MAIN = ['<div class="layui-form layui-border-box {{d.VIEW_CLASS}}" lay-filter="LAY-table-{{d.index}}" style="{{# if(d.data.width){ }}width:{{d.data.width}};{{# } }} {{# if(d.data.height){ }}height:{{d.data.height}}px;{{# } }}"'
			, '{{# if(d.data.skin){ }}lay-skin="{{d.data.skin}}"{{# } }} {{# if(d.data.size){ }}lay-size="{{d.data.size}}"{{# } }} {{# if(d.data.even){ }}lay-even{{# } }}>'
			, '{{# var fixed, right; }}'
	...
	Class.prototype.pullData = function(curr, loadIndex) {
		...
		if(options.url) { //Ajax请求
			if(options.remoteFunc && typeof options.remoteFunc === 'function') {
				options.remoteFunc(options, curr, loadIndex, that);
			} else {
				$.ajax({
	...
	Class.prototype.renderData = function(res, curr, count, sort) {
		...
		if(data.length === 0) {
			//return that.layMain.html('<tr><td class="layui-nodata" colspan="99">无数据</td></tr>');
			//return;
		}
	...
	Class.prototype.events = function() {
		...
		th.on('mousemove', function(e) {
			...
			dict.allowResize = false;//othis.width() - pLeft <= 10; //是否处于拖拽允许区域
	...
	table.checkStatus = function(id) {
		...
		return {
			data: arr //选中的数据
			, isAll: nums > 0 && nums === data.length //是否全选
		};
	};
