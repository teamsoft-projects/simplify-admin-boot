...
layui.define('layer', function(exports) {
	...
	var $ = layui.$
		...
		this.config = {
			...
			invalidTips: function(item, tips) {
				layer.msg(tips, {
					icon: 5
					, shift: 6
				});
	...
	//验证不通过提醒设定
	Form.prototype.invalidTips = function(func) {
		if(func !== undefined && func !== null && typeof func === 'function')
			this.config.invalidTips = func;
	};
			...
			, checkbox: function() {
				...
				checks.each(function(index, check) {
					...
					var reElem = $(['<div class="layui-unselect ' + RE_CLASS[0] + (
						...
						}[skin] || ((check.title.replace(/\s/g, '') ? ('<span>' + check.title + '</span>') : '') + '<i class="layui-icon">&#xe605;</i>')
						, '</div>'].join(''));
	...
	var submit = function() {
		...
		layui.each(verifyElem, function(_, item) {
			...
			// 新增针对Select的校验 add by zhangcc 20180522
			if(othis[0].tagName !== 'SELECT' && othis.is(":hidden")) { // 隐藏的项目不参与校验 add by zhangcc
				return false;
			}
			...
			layui.each(ver, function(_, thisVer) {
				...
				if(verify[thisVer] && (isFn ? tips = verify[thisVer](value, item) : !verify[thisVer][0].test(value))) {
					form.config.invalidTips(item, tips || verify[thisVer][1]);
					return stop = true;
				}
			});
			if(stop) return stop;
